...
const MapComponent = () => {
  ...
  const [graficosVisiveis, setGraficosVisiveis] = useState(false);
  ...
  const handleUpdatePonto = async (id, updatedPonto) => {
    try {
      const convertedId = bufferToUUID(id);
      const response = await api.put(`/${convertedId}`, updatedPonto);
      setPontos(pontos.map(ponto => (ponto._id === convertedId ? response.data : ponto)));
  ...
  const handleDeletePonto = async (id) => {
    try {
      const convertedId = bufferToUUID(id);
      await api.delete(`/${convertedId}`);
      setPontos(pontos.filter(ponto => ponto._id !== convertedId));
  ...
  const bufferToUUID = (buffer) => {
    return uuidStringify(buffer.data);
  };
  ...
  return (
      <div className='map-wrapper'>
        ...
        <div className="hud">
          ...
          <div className="hud-opcoes">
            <button className="btn-dashboard hud-subtitulo" onClick={() => setMostrarFiltros(true)}>Aplicar Filtros de Pesquisa</button>
            <button className="btn-dashboard hud-subtitulo" onClick={() => setGraficosVisiveis(!graficosVisiveis)}>Habilitar Gráficos</button>
          </div>
          {user && (
            <div className="hud-opcoes hud-logout">
              <button className="btn-dashboard hud-subtitulo" onClick={logout}>Logout</button>
            </div>
          )}
        </div>
            ...
            >
              <Popup>
                <div>
                  ...
                  <button className="btn-starwars" onClick={() => handleDeletePonto(ponto._id)}>Deletar</button>
                </div>
              </Popup>
            </Marker>
          ))}
        ...
        {graficosVisiveis && (
          <div className="graficos-sidebar">
            <iframe
              ...
              width="600"
              height="300"
              src="https://charts.mongodb.com/charts-project-0-kqitrpp/embed/charts?id=ddf85dfe-3dc7-4f79-9b20-b77c7feb21b1&maxDataAge=3600&theme=light&autoRefresh=true"
            />
          </div>
        )}
